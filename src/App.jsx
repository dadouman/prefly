...
async function searchYouTubeVideoId(query) {
  // Primary: local Vite proxy (server-side YouTube scraping, no CORS)
  try {
  ...
  // Fallback: Invidious/Piped instances (in case proxy unavailable)
  const fallbacks = [
...
// Cache so repeated plays are instant
const videoIdCache = {};
...
function YouTubePlayer({ item }) {
  ...
  // Reset when item changes (e.g. next duel)
  useEffect(() => {
    setOpen(false);
    setVideoId(null);
    setError(false);
